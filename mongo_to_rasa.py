...
#'mongodb://0.0.0.0/chat_bot'
def fetch_data_from_mongodb():   
    # Se connecter à MongoDB
    client = MongoClient('localhost', 27017)
    db = client['chatbot_index'] 
    collection = db['index'] 
...
append_nlu_file(mongo_data, 'data/nlu.yml')

# Append data to existing stories file
append_stories_file(mongo_data, 'data/stories.yml')

# Append data to existing rules file
append_rules_file(mongo_data, 'data/rules.yml')

# Append data to existing domain file
append_domain_file(mongo_data, 'domain.yml')